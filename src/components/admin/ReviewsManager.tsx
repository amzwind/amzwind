...
export function ReviewsManager() {
  ...
  async function loadData() {
    ...
    const enriched = reviewData.map((r) => ({
      ...r,
      experience_name: expMap.get(r.experience_id) || r.experience_id,
      user_name: userMap.get(r.user_id) || r.user_id,
    }))
  ...
  async function updateStatus(id: string, status: 'approved' | 'rejected') {
    ...
    if (error) setToast({ message: error.message, type: 'error' })
      ...
      setToast({ message: status === 'approved' ? t.reviewsApproved : t.reviewsRejected, type: 'success' })
      await loadData()
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {confirmDelete && (
        <ConfirmModal
          title={t.reviewsDelete}
          message={t.reviewsConfirmDelete}
      ...
      {filtered.length === 0 ? (
        <EmptyState
          icon={<svg className="w-12 h-12 text-gray-300 dark:text-white/10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg>}
        ...
        <div className="space-y-3">
          {filtered.map((review) => (
            <div
              key={review.id}
              className={`bg-white dark:bg-white/[0.03] rounded-2xl border p-5 transition-all ${
                review.status === 'pending'
                  ? 'border-amz-dourado/30'
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-gray-500 dark:text-white/40 mb-2 flex flex-wrap gap-2">
                    <span className="font-medium text-gray-700 dark:text-white/60">{review.user_name}</span>
                    <span>·</span>
                    <span>{review.experience_name}</span>
                    <span>·</span>
                    <span>{new Date(review.created_at).toLocaleDateString('pt-BR')}</span>
                  </div>

                  {review.parent_id && (
                    <span className="text-xs text-amz-dourado font-medium mb-1 inline-block">
                      ↳ {t.reviewsReply}
                    </span>
                  )}

                  {review.comment && (
                    <p className="text-sm text-gray-700 dark:text-white/60 leading-relaxed">{review.comment}</p>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg text-xs font-medium bg-gray-100 dark:bg-white/5 text-gray-500 dark:text-white/30 hover:bg-red-100 hover:text-red-600 dark:hover:bg-red-900/30 dark:hover:text-red-400 transition-colors"
                  >
                    {t.reviewsDelete}
                  </button>
                </div>
