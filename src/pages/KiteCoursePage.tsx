...
export default function KiteCoursePage() {
  ...
  const includesList = [
    t.kiteCourseInclude1,
    t.kiteCourseInclude2,
    t.kiteCourseInclude3,
    t.kiteCourseInclude4,
    t.kiteCourseInclude5,
    t.kiteCourseInclude6,
    t.kiteCourseInclude7,
    t.kiteCourseInclude8,
  ]

  const modules = [
    {
      title: t.kiteCourseModule1Title,
      lessons: 3,
      desc: t.kiteCourseModule1Desc,
    },
    {
      title: t.kiteCourseModule2Title,
      lessons: 4,
      desc: t.kiteCourseModule2Desc,
    },
    {
      title: t.kiteCourseModule3Title,
      lessons: 3,
      desc: t.kiteCourseModule3Desc,
    },
  ]
    ...
    <div className="min-h-screen bg-amz-areia dark:bg-amz-terra-dark">
          ...
          <div className="max-w-5xl mx-auto px-4 pb-10 w-full">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-white/70">
              <span className="flex items-center gap-1.5">
                ...
                {t.kiteCourseTotalHours}
              </span>
              <span className="w-1 h-1 rounded-full bg-white/40" />
              <span>{TOTAL_CLASSES} {t.kiteCourseClasses}</span>
              <span className="w-1 h-1 rounded-full bg-white/40" />
      ...
      <div className="max-w-5xl mx-auto px-4 py-12 space-y-16">
        ...
        <div className="bg-gradient-to-br from-amz-terra via-amz-terra to-amz-terra-dark rounded-3xl p-8 text-white text-center shadow-2xl">
          <p className="text-white/60 text-sm uppercase tracking-widest mb-2">{t.kiteCourseInvestment}</p>
          <p className="text-5xl font-bold font-maybug mb-2">R$ {COURSE_PRICE.toLocaleString('pt-BR')}</p>
          <p className="text-white/70 text-sm">{t.kiteCourseInvestmentSummary}</p>
        </div>

        {/* Description */}
        <div className="space-y-4">
          <h2 className="font-maybug text-2xl text-amz-terra dark:text-amz-areia">{t.kiteCourseAbout}</h2>
          <p className="text-amz-terra-light dark:text-amz-areia/70 leading-relaxed">
            {t.kiteCourseAboutText}
          </p>
        </div>
        ...
        <div className="rounded-3xl overflow-hidden shadow-xl border border-amz-areia-dark/20 dark:border-white/5">
          <div className="relative pb-[56.25%] h-0 bg-black">
            <div className="absolute inset-0 flex items-center justify-center bg-amz-terra-dark/10 dark:bg-white/5">
              <p className="text-amz-terra-light dark:text-amz-areia/60 text-sm">{t.kiteCourseVideoSoon}</p>
            </div>
          </div>
        </div>
        ...
        {/* Includes */}
        <div>
          <h2 className="font-maybug text-2xl text-amz-terra dark:text-amz-areia mb-6">{t.kiteCourseWhatIncluded}</h2>
        ...
        {/* Modules */}
        <div>
          <h2 className="font-maybug text-2xl text-amz-terra dark:text-amz-areia mb-6">{t.kiteCourseModules}</h2>
          <div className="space-y-4">
            {modules.map((mod, i) => (
              <div
                key={mod.title}
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${activeModule === i ? 'bg-white/20 text-white' : 'bg-amz-dourado/10 text-amz-dourado'}`}>
                    {mod.lessons} {t.kiteCourseClasses}
                  </span>
                </div>
        ...
        {/* Gallery */}
        <div>
          <h2 className="font-maybug text-2xl text-amz-terra dark:text-amz-areia mb-6">{t.kiteCourseGallery}</h2>
        ...
        <div className="bg-white dark:bg-white/[0.03] rounded-3xl p-8 border border-amz-areia-dark/10 dark:border-white/5 shadow-lg">
          <h2 className="font-maybug text-2xl text-amz-terra dark:text-amz-areia mb-2">{t.kiteCourseSchedule}</h2>
          <p className="text-sm text-amz-terra-light dark:text-amz-areia/50 mb-6">
            {t.kiteCourseScheduleDescription}
          </p>
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-4 items-center justify-between">
            <div>
              <p className="text-sm text-amz-terra-light dark:text-amz-areia/50">{t.kiteCourseTotalInvestment}</p>
            ...
            >
              {checkIn ? t.kiteCourseScheduleButton : t.kiteCourseSelectDate}
            </button>
          </div>
        </div>
        ...
        {/* Reviews Placeholder */}
        <div>
          <h2 className="font-maybug text-2xl text-amz-terra dark:text-amz-areia mb-6">{t.kiteCourseReviews}</h2>
          <div className="bg-white dark:bg-white/5 rounded-2xl p-8 border border-amz-areia-dark/10 dark:border-white/5 text-center">
            ...
            <p className="text-sm text-amz-terra-light dark:text-amz-areia/50">{t.kiteCourseReviewsEmpty}</p>
          </div>
        </div>
      </div>
